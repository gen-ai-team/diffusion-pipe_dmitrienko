...
class WanModelFromSafetensors(WanModel):
    ...
    def from_pretrained(
        ...
        state_dict = {
            re.sub(r"^model\.diffusion_model\.", "", k): v
        ...
        for name, param in model.named_parameters():
            dtype_to_use = (
                torch_dtype
                if any(keyword in name for keyword in KEEP_IN_HIGH_PRECISION)
                else transformer_dtype
            )
            set_module_tensor_to_device(
                model, name, device="cpu", dtype=dtype_to_use, value=state_dict[name]
            )
...
class WanPipeline(BasePipeline):
    ...
    def __init__(self, config):
        ...
        target_modules_type = self.config["adapter"].get("target_modules_type", "default")
        if target_modules_type == "default":
            self.adapter_target_modules = ["WanAttentionBlock"]
        
        elif target_modules_type == "nocfg":
            self.adapter_target_modules = [ 
                "ffn.0", "ffn.2", "self_attn.q", "self_attn.k", "self_attn.v", "self_attn.o",
        ...
        if not self.i2v and model_dim == 1536:
            ...
            raise RuntimeError(
                f"Could not autodetect model variant. model_dim={model_dim}, i2v={self.i2v}"
            )

        # This is the outermost class, which isn't a nn.Module
        t5_model_path = (
            self.model_config["llm_path"]
            if self.model_config.get("llm_path", None)
            else os.path.join(ckpt_dir, wan_config.t5_checkpoint)
        )
    ...
    def load_diffusion_model(self):
        ...
        if transformer_path := self.model_config.get("transformer_path", None):
            if transformer_path.endswith(".safetensors"):
                ...
                print(f"LOAD FROM {transformer_path}")
                weight_dict = torch.load(transformer_path, weights_only=False)
                if 'dit' in weight_dict:
                    weight_dict = weight_dict['dit']
    ...
    def get_call_vae_fn(self, vae_and_clip):
        def fn(tensor):
            ...
            if clip is not None:
                assert (
                    tensor.ndim == 5
                ), f"i2v must train on videos, got tensor with shape {tensor.shape}"
...
class InitialLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        e = self.time_embedding(
            sinusoidal_embedding_1d(self.freq_dim, t).to(x.device, torch.float32)
        )
